...
async function uploadAsset(containerClient, asset) {
  ...
  if (asset.content) {
    if (await compression.canBeCompressed(asset, blobContentType)) {
      compressedVersions = await compression.compress(asset.content);
    }

    logCb = () => {
      logger.info(
        'Setting content type to %s for blob %s',
        blobContentType,
        asset.name
      );
  ...
  if (compressedVersions) {
    ...
    await uploadAsset(containerClient, {
...
async function registerAssetsToBlob(settings, assets) {
  ...
  try {
    ...
    logger.info('%s registered in blob storage.', settings.name);
  } catch (err) {
    logger.error(err);
    throw err;
  }
...
async function registerEntityToTable(settings, entity) {
  const tableClient = new azureTables.TableClient(
    ...
    new azureTables.AzureNamedKeyCredential(
      settings.azureStorageAccount,
      settings.azureStorageAccessKey
    )
  );
  ...
  try {
    ...
    logger.info('%s registered in table storage.', settings.name);
  } catch (err) {
    logger.error(err);
    throw err;
  }
