...
const getDistAssets = (includeContent, isStaticClient, version, hashFileNames) => {
  ...
  if (isStaticClient) {
    ...
    if (versionMajor) {
      ...
      return {
        scripts: merged.filter(x => x.type === 'script'),
        styleSheets: merged.filter(x => x.type === 'styleSheet')
      };
    }

    return {
      scripts: assets.filter(x => x.type === 'script'),
      styleSheets: assets.filter(x => x.type === 'styleSheet')
    };
  }
